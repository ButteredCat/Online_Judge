...
private:
    ...
    vector<vector<int> > getBottomUpValues(const vector<vector<TreeNode*> > &tree) {
        vector<vector<int> > treeValues;
        for(auto eachLevel = tree.rbegin(); eachLevel != tree.rend(); ++eachLevel) {
